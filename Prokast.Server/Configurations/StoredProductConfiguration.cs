...
namespace Prokast.Server.Configurations
{
    public class StoredProductConfiguration: IEntityTypeConfiguration<StoredProduct>
    {
        public void Configure(EntityTypeBuilder<StoredProduct> builder)
        {
            ...
            builder.HasOne(x => x.Warehouse).WithMany(y => y.StoredProducts).HasForeignKey(x => x.WarehouseID).OnDelete(DeleteBehavior.Cascade);
            builder.HasOne(x => x.Product).WithOne(y => y.StoredProduct).HasForeignKey<StoredProduct>(z => z.ProductID).OnDelete(DeleteBehavior.Cascade);
        }
    }
}
