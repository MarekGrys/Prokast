...
const PhotoComponent = ({
    ...
    const updateLocalDescription = (updatedData: Photo) => {
        setPhotos((prev) =>
            prev.map((photo) =>
                photo.localId === updatedData.localId ? { ...photo, ...updatedData } : photo
            )
        );
    };
    ...
    const removeLocalDescription = (localIdToRemove: number) => {
        setPhotos((prev) =>
            prev.filter((photo) => photo.localId !== localIdToRemove)
        );
    };
    useEffect(() => {
        if (onChange) {
                    ...
                    <div className="bg-white rounded-lg p-6 w-[400px] shadow-lg space-y-4">
                        ...
                        <div className="flex justify-end gap-3 pt-2">
                            <button
                                type="button"
                                onClick={() => {
                                    if (!selectedPhoto?.localId) {
                                        console.error("Brak wybranego elementu do usunięcia.");
                                        return;
                                    }
                                    removeLocalDescription(selectedPhoto.localId);
                                    setIsDeleteOpen(false);
                                    console.log("Aktualne opisy po usunięciu:", photos);
                                }}
                                className="px-4 py-2 bg-green-600 text-white rounded hover:bg-green-700"
                            >
                                Tak
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    <div className="bg-white rounded-lg p-6 shadow-lg space-y-4 relative">
                        ...
                        <img
                            src={
                                selectedPhoto
                                    ? `data:${selectedPhoto.contentType};base64,${selectedPhoto.value}`
                                    : undefined
                            }
                            alt="Podgląd zdjęcia"
                            className="mt-2 h-32 object-cover border"
